import { getPeriodKey, parseDate, toISODate } from './dateUtils.js';

// Margin = revenue - cost - labor - ad - fee - vat
export function computeMargin(row) {
  const expenses = (row.cost || 0) + (row.labor || 0) + (row.ad || 0) + (row.fee || 0) + (row.vat || 0);
  const margin = (row.revenue || 0) - expenses;
  const marginRate = row.revenue ? (margin / row.revenue) * 100 : 0;
  return { margin, marginRate, expenses };
}

export function aggregate(rows) {
  const agg = {
    revenue: 0,
    cost: 0,
    labor: 0,
    ad: 0,
    fee: 0,
    vat: 0,
    margin: 0,
    count: rows.length
  };
  for (const r of rows) {
    agg.revenue += r.revenue || 0;
    agg.cost += r.cost || 0;
    agg.labor += r.labor || 0;
    agg.ad += r.ad || 0;
    agg.fee += r.fee || 0;
    agg.vat += r.vat || 0;
  }
  agg.margin = agg.revenue - agg.cost - agg.labor - agg.ad - agg.fee - agg.vat;
  agg.marginRate = agg.revenue ? (agg.margin / agg.revenue) * 100 : 0;
  agg.grossProfit = agg.revenue - agg.cost;
  agg.grossRate = agg.revenue ? (agg.grossProfit / agg.revenue) * 100 : 0;
  return agg;
}

export function groupByPeriod(rows, granularity) {
  const groups = new Map();
  for (const r of rows) {
    const key = getPeriodKey(r.date, granularity);
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(r);
  }
  const result = [];
  for (const [key, items] of groups) {
    const agg = aggregate(items);
    result.push({ period: key, ...agg });
  }
  result.sort((a, b) => a.period.localeCompare(b.period));
  return result;
}

export function groupByBusiness(rows) {
  const groups = new Map();
  for (const r of rows) {
    const k = r.business || '미지정';
    if (!groups.has(k)) groups.set(k, []);
    groups.get(k).push(r);
  }
  return Array.from(groups.entries())
    .map(([business, items]) => ({ business, ...aggregate(items) }))
    .sort((a, b) => b.revenue - a.revenue);
}

export function groupByChannel(rows) {
  const groups = new Map();
  for (const r of rows) {
    const k = r.channel || '기타';
    if (!groups.has(k)) groups.set(k, []);
    groups.get(k).push(r);
  }
  return Array.from(groups.entries())
    .map(([channel, items]) => ({ channel, ...aggregate(items) }))
    .sort((a, b) => b.revenue - a.revenue);
}

export function filterRows(rows, { from, to, businesses, channels }) {
  return rows.filter((r) => {
    if (from && r.date < from) return false;
    if (to && r.date > to) return false;
    if (businesses && businesses.length && !businesses.includes(r.business)) return false;
    if (channels && channels.length && !channels.includes(r.channel)) return false;
    return true;
  });
}

export function getDateRange(rows) {
  if (!rows.length) return { min: null, max: null };
  let min = rows[0].date;
  let max = rows[0].date;
  for (const r of rows) {
    if (r.date < min) min = r.date;
    if (r.date > max) max = r.date;
  }
  return { min, max };
}

export function getUniqueValues(rows, field) {
  const s = new Set();
  for (const r of rows) {
    const v = r[field];
    if (v) s.add(v);
  }
  return Array.from(s).sort();
}

// Previous-period comparison
export function previousPeriodRange(from, to) {
  const f = parseDate(from);
  const t = parseDate(to);
  if (!f || !t) return null;
  const days = Math.round((t - f) / 86400000) + 1;
  const prevTo = new Date(f);
  prevTo.setDate(prevTo.getDate() - 1);
  const prevFrom = new Date(prevTo);
  prevFrom.setDate(prevFrom.getDate() - (days - 1));
  return { from: toISODate(prevFrom), to: toISODate(prevTo) };
}
