...
const FIELD_ALIASES = {
  ...
  spec: ['규격', '옵션', '사이즈'],
  quantity: ['수량', 'qty'],
  recipient: ['수취인명', '수취인', '수령인', '수령자', '받는사람', '받는분', '수하인', '받으실분'],
  phone: ['수취인연락처', '수령자휴대전화', '수령자전화번호', '수령자연락처', '휴대폰번호', '연락처', '전화번호', '핸드폰', '휴대전화'],
...
function parseRawOrderSheet(json, headerIdx) {
  ...
  const c = {
    ...
    shipping: idx('배송비'),
    orderNo: idx('주문번호'),
    recipient: idxAny('수취인명', '수취인', '수령인', '수령자', '받는사람', '받는분', '수하인', '받으실분'),
    phone: idxAny('수취인연락처', '수취인전화번호', '수령자휴대전화', '수령자전화번호', '수령자연락처', '휴대폰번호', '연락처', '전화번호'),
